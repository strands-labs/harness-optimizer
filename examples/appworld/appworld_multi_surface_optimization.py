"""Example: optimize AppWorld's system prompt, skill library and tool descriptions TOGETHER.
...
```
appworld_multi_surface_optimization.py         appworld_runtime/  (the deployable runtime)
  ...
  AppWorldReward       skills_folder, tool_descriptions, exp_id}
        │
...
Compare with ``appworld_agentcore_optimization.py`` (prompt only) and
``appworld_skill_library_optimization.py`` (skills only) on the same runtime.
...
from appworld_skill_library_optimization import pack_inline  # noqa: E402
...
OUTPUT_ROOT = os.getenv("MULTI_OUTPUT_ROOT", "./appworld_multi_surface_runs")
...
# appworld_runtime/dump_tool_descriptions.py. The reflector starts from this map.
TOOL_DESCRIPTIONS_YAML = os.getenv(
    "TOOL_DESCRIPTIONS_YAML",
    os.path.join(HERE, "appworld_runtime", "prompts", "tool_descriptions.yaml"),
)
...
def main():
    ...
    formula = MultiSurfaceFormula(
        system_prompt=SystemPromptFormula(system_prompt=APPWORLD_SYSTEM_PROMPT),
